...
jlong pb_jni_nativeNewRuntime(JNIEnv *env, jclass clazz, jstring routing_json, jstring root_dir) {
    (void)clazz;
...
void pb_jni_nativeFreeRuntime(JNIEnv *env, jclass clazz, jlong runtime_ptr) {
    (void)env;
    (void)clazz;
    if (runtime_ptr != 0) {
...
void pb_jni_nativeUpdateRouting(JNIEnv *env, jclass clazz, jlong runtime_ptr, jstring routing_json) {
    (void)clazz;
    if (runtime_ptr == 0) {
        return;
    }
...
jlong pb_jni_nativeCreateEngine(
...
jstring pb_jni_nativeGenerateTextAsync(
...
void pb_jni_nativeCancelOperation(
