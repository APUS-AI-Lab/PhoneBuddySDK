...
| `codex` | `responses` (`/v1/responses`) | `codex_cli_rs/{version} ({os}; {arch}) codex_cli` | <ul><li>`originator: codex_cli_rs`</li><li>`openai-beta: responses=true`</li><li>`session-id`, `thread-id`, `x-client-request-id`, `x-codex-installation-id`, `x-codex-window-id`</li><li>Logical-turn-scoped sticky routing via `x-codex-turn-state`</li><li>`Authorization: Bearer <key>`</li><li>Standard OpenAI Responses payload with `parallel_tool_calls`, `tool_choice`, `stream_options`, `text.verbosity`, and `client_metadata`</li></ul> |
